...
// ------------------------------------------------------------
// Запрос на завершение (депозит) заказа
// ------------------------------------------------------------

type DepositOrderRequest struct {
	// Номер заказа в платежном шлюзе (обязательный) [1..36]
	OrderID string `json:"orderId"`

	// Сумма завершения в минимальных единицах валюты (обязательный)
	// Если указать 0, депозит будет на всю сумму заказа
	Amount int `json:"amount"`

	// Язык ответа (ISO 639-1: ru, en, by, kz, kk) (необязательный)
	Language string `json:"language,omitempty"`
